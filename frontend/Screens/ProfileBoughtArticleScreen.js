...
const ProfileBoughtArticleScreen = (props) => {
  ...
  var handleClick = async (value) => {
    ...
    const data = await fetch(`${DISTANT_HOST}/orders/receive-order?idArticle=${value._id}&sellerToken=${value.sellerToken}&productPrice=${value.price}`) 
  ...
  useEffect(() => {
    ...
    const findProducts = async () => {
      ...
      const data = await fetch(`${DISTANT_HOST}/articles/get-article-by-buyer?buyerToken=${props.takeToken}`)
      const body = await data.json()
